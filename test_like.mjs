...
async function test() {
  ...
  const res1 = await fetch(`${SUPABASE_REST_URL}/kv_store_5b740c2f?select=*&key=like.user:%`, { headers: restHeaders });
  ...
  console.log("\nTesting like.user:*");
  const res2 = await fetch(`${SUPABASE_REST_URL}/kv_store_5b740c2f?select=*&key=like.user:*`, { headers: restHeaders });
  console.log("Res2 status:", res2.status);
  if (res2.ok) console.log("Res2 length:", (await res2.json()).length);
  else console.log("Res2 error:", await res2.text());
}
